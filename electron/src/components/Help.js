...
const Help = ({ setCurrentPage, isDarkMode, language }) => {
    const [showDocs, setShowDocs] = (0, react_1.useState)(false);
    ...
    return (<div className={`h-screen p-8 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      <div className="max-w-3xl mx-auto">
        <button onClick={() => setCurrentPage('dashboard')} className={`mb-4 ${isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-brown-600 hover:text-brown-700'} flex items-center`}>
        ...
        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-8 shadow-lg`}>
          <section className="mb-8">
            <h2 className="text-xl font-semibold mb-4 flex items-center">
              <lucide_react_1.Book size={24} className="mr-2"/>
              {t.documentation}
            </h2>
            ...
            <button onClick={() => setShowDocs(true)} className={`${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white font-bold py-2 px-4 rounded`}>
              {t.viewDocumentation}
            </button>
          </section>
          ...
          <section className="mb-8">
            <h2 className="text-xl font-semibold mb-4 flex items-center">
              <lucide_react_1.Video size={24} className="mr-2"/>
              {t.videoTutorials}
            </h2>
          ...
          <section className="mb-8">
            <h2 className="text-xl font-semibold mb-4 flex items-center">
              <lucide_react_1.MessageCircle size={24} className="mr-2"/>
              {t.faq}
            </h2>
            <p className="mb-4">Trova risposte alle domande più comuni su Gamebook Studio.</p>
            <button onClick={() => setShowFaq(true)} className={`${isDarkMode ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-yellow-500 hover:bg-yellow-600'} text-white font-bold py-2 px-4 rounded`}>
              {t.viewFaq}
            </button>
          </section>
          ...
          <section>
            <h2 className="text-xl font-semibold mb-4 flex items-center">
              <lucide_react_1.Mail size={24} className="mr-2"/>
              {t.contactSupport}
            </h2>
      ...
      {/* Documentation Modal */}
      {showDocs && (<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto relative`}>
            <button onClick={() => setShowDocs(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700">
              <lucide_react_1.X size={24}/>
            </button>
            <h2 className="text-3xl font-bold mb-8">{docs.title}</h2>
            {docs.sections.map((section, index) => (<div key={index} className="mb-8">
                <h3 className="text-xl font-semibold mb-4">{section.title}</h3>
                {section.content.map((paragraph, pIndex) => (<p key={pIndex} className="mb-2">{paragraph}</p>))}
              </div>))}
            <button onClick={() => setShowDocs(false)} className={`mt-4 ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white font-bold py-2 px-4 rounded`}>
              {t.close}
            </button>
          </div>
        </div>)}
      ...
      {/* FAQ Modal */}
      {showFaq && (<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto relative`}>
            <button onClick={() => setShowFaq(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700">
              <lucide_react_1.X size={24}/>
            </button>
            <h2 className="text-3xl font-bold mb-8">{t.faq}</h2>
            {currentFaqs.map((faq, index) => (<div key={index} className="mb-6">
                <h3 className="text-lg font-semibold mb-2">{faq.question}</h3>
                <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{faq.answer}</p>
              </div>))}
            <button onClick={() => setShowFaq(false)} className={`mt-4 ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white font-bold py-2 px-4 rounded`}>
              {t.close}
            </button>
          </div>
        </div>)}
